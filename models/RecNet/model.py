...
class ReconstructorAttention(nn.Module):
    ...
    def forward(self,hidden,feats):
        ...
        shape of feats (batch size,feat_len,feat_size)  #(100,feat_len(decoder_time_step,varry),512)
        '''
        Wh = self.decoder_projection(hidden)  
        Uv = self.encoder_projection(feats)   
        Wh = Wh.unsqueeze(1).expand_as(Uv)
...
class RecNet(nn.Module):
    def __init__(self,voc,cfg,path):
        ...
        if self.reconstructor_type == 'global':
            self.global_reconstructor = GlobalReconstructor(cfg)
        ...
        if self.reconstructor_type == 'local':
            self.local_reconstructor = LocalReconstructor(cfg)
    ...
    def forward_global_reconstructor(self,video_features,decoder_hidden):
        ...
        for l in range(decoder_time_step):
            ...
            output,global_hidden = self.global_reconstructor(decoder_hidden[l],
                                                             mean_decoder_hidden,global_hidden)
            global_output.append(output)
            
        global_output = torch.stack(global_output,0) # (T,B,1536)
        mean_global_output = global_output.mean(dim=0)
    ...
    def train_iter(self,utils,input_variable, target_variable, mask,max_target_len,use_teacher_forcing):
        ...
        if self.cfg.training_stage == 2:
            ...
            if self.cfg.reconstructor_type == 'global':
                reconstruction_loss = self.reconstruction_loss_calculate(self.forward_global_reconstructor(input_variable,
                                                                                    decoder_hidden_list))
            if self.cfg.reconstructor_type == 'local':
                reconstruction_loss = self.reconstruction_loss_calculate(self.forward_local_reconstructor(input_variable,
                                                                                    decoder_hidden_list))
                
            total_loss = likelihood_loss + self.lmda*reconstruction_loss
            total_loss.backward()
        ...
        return sum(print_losses) / n_totals, reconstruction_loss
